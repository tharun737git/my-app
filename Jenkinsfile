node{
   ...
   sh 'docker build -t manojsb24/myweb:0.0.2 .'
   }
   stage('Docker Image Push'){
   withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
   sh "docker login -u manojsb24 -p ${dockerPassword}"
    }
   sh 'docker push manojsb24/myweb:0.0.2'
   }
   ...
   sh 'docker run -d -p 8090:8080 --name tomcattest manojsb24/myweb:0.0.2' 
   }
}
}
